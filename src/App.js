...
function App() {
  ...
  const registerNewPoints = () => {
    ...
    if (!data) {
      ...
      notifySuccess(`Os pontos foram adicionados!`)
    } else {
  ...
  const registerNewPointsTextArea = () => {
    try {
      ...
      const refactory = responseJson.x.map((e, idx) => ({
        x: e,
        y: responseJson.y[idx],
      }))
      setPoints(refactory)
      notifySuccess(`Os pontos foram adicionados!`)
    } catch (e) {
      ...
      <Side id="SideBar">
        <div className="contentData">
          <Header>
            ...
            <p>
              Uma técnica de otimização matemática que procura encontrar o
              melhor ajuste para um conjunto de dados tentando minimizar a soma
              dos quadrados das diferenças entre o valor estimado e os dados
              observados.
            </p>
            <p>
              Adicione novos valores na tabela para realizar o cálculo da
              equação:
            </p>
          </Header>
          ...
          <ContentInputJson>
            ...
            <p>
              ...
              <a className="clear" onClick={() => setTextExample("")}>
                Limpar
              </a>
            </p>

            <button onClick={registerNewPointsTextArea}>
          ...
          {points.length > 0 && (
            <>
              ...
              />
              <TextResponse>
                D({additionalInformation?.B1}; {additionalInformation?.B0}) ={" "}
                {additionalInformation?.SomaQuadradoDistacia}
              </TextResponse>
            </>
          )}
        </div>
        ...
        <Footer>
          <p>
            Desenvolvido por{" "}
            <a target="_blank" href="https://github.com/samlucas">
              Samuel Lucas
            </a>
          </p>
          <img src={IconGitHub} alt="githubicon" />
          <br />
        </Footer>
      </Side>
